 ...
 * record written from fault context is committed to SRAM immediately (no cache
 * maintenance) and survives every *system* reset (reboot / fault / IWDG / WWDG /
 * NRST / LPWR, RM0385 5.1.1).  CMSIS Reset_Handler initialises only .data/.bss,
 * never .log_noinit, so the bytes carry over; a *power* reset (POR/PDR/BOR,
 * 5.1.2) loses them, which log_init() detects via the magic and re-initialises.
 *
...
static const char *reset_cause_str(uint32_t csr)
{
	...
	if (csr & RCC_CSR_SFTRSTF)  return "SFT";
