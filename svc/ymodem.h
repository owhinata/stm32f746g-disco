...
/**
 * @file    ymodem.h
 * @brief   Clean-room YMODEM-CRC batch sender (svc/ layer, issue #50).
 *
 ...
 * as vtables, so this core has NO dependency on the shell, ThreadX, HAL or
 * FileX -- it sits in the freestanding svc/ layer next to fmt.c/log.c and is
 * unit-tested on the host (shell/test/test_ymodem.c) against mock IO/source.
 *
 * The shell wires @ref ym_io to cli_read_byte()/cli_write() and @ref ym_source
 * to camera_frame_read() (camera send) or fx_file_read() (xfer send sd|fs).
 *
