 ...
 * NRST / LPWR, RM0385 5.1.1) and can be replayed after reboot with `dmesg`.  The
 * fault handler (src/fault.c) records the crash here as well, so a HardFault is
 * still visible after the watchdog or a manual reset.  Clean-room: the *concept*
 * is borrowed from NuttX ramlog / Zephyr logging, but no code is reused.
 *
 ...
 * cause.  Call once from bsp_init() BEFORE fault_init() so the fault handler
 * always finds a valid ring.  log_write() is a no-op until this returns.
 */
void log_init(void);
