 ...
 *
 * A dedicated ThreadX thread polls the FT5336 (port/touch) ~60 Hz and turns the
 * first touch point into GUIX pen events (GX_EVENT_PEN_DOWN / _PEN_DRAG /
 * _PEN_UP) via gx_system_event_send().  Coordinates are panel pixels straight
 * from touch_read() (no scaling -- see touch.h).  The thread is created once and
 * then parked/un-parked by an `active` flag: gui stop clears it so the thread
 * sleeps OUTSIDE any I2C transaction (never tx_thread_suspend(), which could
 * stop it while it holds the touch mutex).
 */
#ifndef GUIX_TOUCH_H
#define GUIX_TOUCH_H
