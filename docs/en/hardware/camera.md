...
- **Retrieval**: for a **plain `camera stream` (no external sink)** each JPEG publish mirrors the finalised frame into the (idle during streaming) `cam_frame`, so `camera save` / `camera send` can pull the latest JPEG frame during or after the stream. **While an external sink is attached (`net mjpeg start` MJPEG-over-HTTP #78 / GUIX live preview #56) this per-frame ~19.5 KB non-cacheable SDRAM mirror is skipped (#82)** -- it roughly halves the `cam_prod` per-frame copy cost and makes the JPEG path match the raster preview path (which never mirrors). During such a stream `camera save` / `camera send` return the **last non-external-sink frame** (or `CAM_ERR_NO_FRAME` if none).
