...
!!! note "Phase 状況"
    P1 の **null backend 土台** と **X-CUBE-AI(`stedgeai`) backend** を実装済み
    （後者は MNIST int8 サンプルで実機検証: ランタイム実行 + cacheable arena）。
    BlazeFace デコード（#8）、SD からのモデルロード（P2）、TFLM backend（P3）、
    GUIX オーバレイ（P4）は後続。
