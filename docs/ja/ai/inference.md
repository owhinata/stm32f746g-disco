...
!!! note "Phase 状況"
    ...
    **P4 完了** — GUIX ライブプレビュー上に顔 bbox を描画（#83）。**#100 で `gui overlay`
    コマンドは廃止**し、`ai stream` 稼働中は preview に常時 bbox を描画するようになった
    （overlay の FEED モードは撤去、nncam は base capture の subscriber）。
    **P3 完了** — TFLM(tflite-micro) backend。M1 spike（#86）で C++ 成立性を実証後、
    **M2（#88）で BlazeFace-front 128 int8 を本統合**。同じ `.tflite` を stedgeai と
    TFLM 双方が実行し、`ai info`/`ai bench`/`ai stream` が TFLM ランタイムで
...
- **SD 専用**: builtin モデルは持たない。`ai model load network_rel.bin` するまでモデル無し
  （`ai run`/`ai stream` は未ロードを明示エラーで拒否）。`ai model builtin` は
  アンロード（`(none)`）。
