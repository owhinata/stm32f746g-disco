...
ファイルシステム（FileX FAT、#34/#36）コマンド — `fs`(QSPI) と共通コアを共有:

```
sd format [exfat] yes  全カードを FAT32 superfloppy にフォーマット（破壊的、#36）
...
- **`sd format yes`**（#36）: 全カードを **FAT32 superfloppy** にフォーマット（破壊的、`yes` 必須）。空カードでも可（ボード単体でカード初期化）。`sectors/cluster` は容量別に自動選択し FAT32 を保証、format 後に FAT32 を確認。**exFAT は本ビルド非対応** → `sd format exfat yes` はスタブ（[#35](https://github.com/owhinata/stm32f746g-disco/issues/35)）。
- **hot-plug**（#36）: マウント中にカードを抜くと次の `sd` コマンドが検出して退避（flush せず `fx_media_abort`）し "no card"。挿し直し/別カードで次コマンドが remount（旧 BPB を使い回さない）。PC13 検出ピンの polling（EXTI 不使用）。
