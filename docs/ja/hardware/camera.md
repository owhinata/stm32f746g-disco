...
!!! danger "30fps（48MHz）は LTDC scanout 停止が前提"
    ...
    **残る制約（TOCTOU）**: 30fps stream を `lcd off` 下で開始した後に `lcd on` すると競合が復活して OVR → stream は **graceful auto-stop**（`stopped (overrun)` / `ovr dcmi` に計上）する。30fps stream 中は `lcd on` しないこと。LTDC scanout を削って「プレビューでも 30fps」を狙う路線は別スコープ（#59/#65）。

!!! warning "VTS と AEC 最大露光のクランプ"
    VTS が AEC 最大露光行数（`0x3A02/03` と `0x3A14/15`、既定 0x3D8=984）を下回ると露光がクリップして暗化/バンディングする。露光連動の VTS 再クランプは両系統を read-back し `VTS ≥ max + margin` を保証する（night mode が上限を伸ばしたとき VTS を上げ fps が下がる）。再クランプは **AEC 上限/ base VTS を変える操作 — night 切替・解像度/フォーマット切替・configure — でのみ**実行し、SDE/ジオメトリの画質 setter（brightness…zoom）は VTS を触らない＝fps を落とさない（#70）。vendored `OV5640_NightModeConfig(DISABLE)` は auto-frame-rate ビットしか戻さないので、**night off では `port/camera` 側で両系統を 0x3D8 に復帰**する — これが無いと上限が night 値のまま居残り、実効 VTS（と fps）が戻らない。`camera info` の `fps target` は VTS をセンサから read-back するので、この実効（再クランプ後）値を反映し `camera stream stats` の実 fps と一致する（#71）。fps 表の値は**実機 `camera stream stats` で要調整**の出発点である（帯域競合下の最適化は #59）。
...
- **取り出し**: **plain な `camera stream`（外部 sink なし）** では JPEG publish 毎に確定フレームを（streaming 中は idle な）`cam_frame` へミラーするので、`camera save` / `camera send` が streaming 中・停止後とも最新 JPEG フレームを取り出せる。**外部 sink 付き配信中（`net mjpeg start` の MJPEG-over-HTTP #78 / GUIX ライブプレビュー #56）は、この per-frame ~19.5 KB の非キャッシュ SDRAM ミラーを skip する（#82）**：`cam_prod` の per-frame コピー負荷をほぼ半減させ、raster プレビュー経路（元々ミラーしない）と挙動を一致させるため。この間の `camera save` / `camera send` は**最後に外部 sink 無しで撮ったフレーム**（無ければ `CAM_ERR_NO_FRAME`）を返す。
- **プレビュー非対応**: F746 に JPEG decode 経路は無いため、`camera format jpeg` 中はカメラ UI のライブプレビュー（boot / `gui start`）が明示拒否される（`camera format rgb565` 後に preview）。本命の消費先は #49（ネット streaming = JPEG 圧縮で高解像も帯域内）。
