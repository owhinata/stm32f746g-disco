...
| `Ctrl+c` | 入力行をキャンセル（任意の escape 途中からも復帰、§9）。実行中コマンドの協調キャンセルは下記参照（#16） |
| `Enter`（`\r` / `\n`） | 行を dispatch（`\r\n` は 1 回） |
...
実行中コマンドのキャンセル（#16）: 走行中の長時間 / 大量出力コマンドも `Ctrl+c` で中断できる
（**協調的**＝コマンドが `cli_cancel_requested()` を覗くか `cli_sleep()` で待っている場合に効く。
コアはハンドラスレッドを強制終了しない）。中断後は入力行キャンセルと同じく `^C` を表示してプロンプト
へ復帰し、実行中に打たれた type-ahead は破棄する。書き方は
[コマンド登録](shell-registration.md)の「協調的キャンセル」を参照。`coremark` は単一ブロッキング
呼び出しのため中断**不可**。
