...
| `Enter`（`\r` / `\n`） | 行を dispatch（`\r\n` は 1 回） |
| `↑` / `↓`、`Ctrl+p` / `Ctrl+n` | 履歴（#9 は no-op スタブ、#10 で固定リング） |
...
`↑`/`↓`・`Ctrl+p`/`Ctrl+n` は `cli_history_prev/next()` を、dispatch は `cli_history_add()` を
呼ぶが、#9 では `shell/core/cli_history.c` の **no-op スタブ**。#10 は同ファイルの中身を
固定リング（§8: 512 B, FIFO, 直前重複排除）に差し替えるだけでよい。
