...
- コンソール有効後: printf を shell と**同じ TX リング**へ投入（満杯時は有界スピン、TX ISR が
  裏で drain）→ 各 USART の TX 所有者を 1 本化。
- 有効前（起動直後の boot ログ等、`g_uart_console==NULL` / `!enabled`）: `huart1` へ
  `HAL_UART_Transmit` polling。IT TX が未 arm なので衝突しない。
- **LF→CRLF**: 有効後の `_write` は printf 出力中の bare `\n` を `\r\n` に変換する（直前が
  `\r` なら二重化しない）。`coremark` の `\n` 改行レポートが端末側変換（`--imap lfcrlf`）無しでも
  崩れない。shell の `cli_print` は `write()` 経由で `_write` を通らないため無関係。

### printf の呼び出し元端末ルーティング（#18）

`_write` は **実行中スレッドの所有 shell インスタンス**を `cli_current_instance()`
（`shell/core/cli_core.c` の thread→instance レジストリ）で解決し、そのインスタンスの UART TX
リングへ流す。これにより printf 経由出力 ― とりわけ `coremark` のレポート（`ee_printf` =
`printf`、呼び出し元 shell スレッドで同期実行）― が **操作中の端末に追従**する。解決できない
文脈、すなわち **ISR**（IPSR≠0 を判定）/ **スケジューラ起動前**（起動バナー）/ **非 shell
スレッド** / **非 UART transport** では `NULL` となり、従来どおり `g_uart_console`（最後に
init した backend）へフォールバックする。レジストリは `cli_start()` がスレッド生成の**前**に
登録し（自動起動スレッドが未登録で走るレースを排除）、`cli_thread_entry` の全 exit で解除する。
容量は `CLI_THREAD_MAP_MAX`（既定 = `CLI_MAX_INSTANCES`）。

!!! note "現状は単一 USART1"
    backend の `uart_enable`/`USART1_IRQHandler`/HAL コールバックは `g_uart_console` 固定で、
    TxComplete ISR が別インスタンスのリングを advance できないため、**2 本目の UART
    インスタンスはまだ追加していない**（追加には IRQ/コールバックの per-instance 振り分けと、
    同一 `huart` 二重バインド検出が前提）。本ルーティング機構と `_write` retarget は将来の
    マルチ UART 化とバックグラウンド実行（#25）の土台で、単一 USART1 では挙動は従来と同一。

!!! warning "printf の行 atomicity"
    `_write` は per-instance TX mutex（`tx_lock`）を**取らない**（ISR / カーネル起動前 / 任意
    スレッドから呼べる必要があるため）。よって printf と「別スレッドから同一インスタンスへの
    `cli_print`」は**行単位では混ざり得る**（TX リング自体は PRIMASK 保護で破壊されない）。
    `coremark`（printf と shell が同一スレッドで直列）や単一インスタンスでは発生しない。
...
shell をライブラリ化した `shell` アプリ本体・`flash` ターゲット（当時は `flash-shell`）・dummy 2nd instance は #8。
...
- **実機 verify**（#7 では一時配線, commit せず）: `app_threadx.c` へ一時的に backend を結線して
  `flash` → `picocom -b 115200 /dev/ttyACM0` で プロンプト表示・コマンドエコー・
