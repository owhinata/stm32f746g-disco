...
ボード搭載 4.3″ 480×272 LCD（LTDC、#52/#53）と FT5336 容量タッチ（I2C3、#54）の上に、**Eclipse ThreadX GUIX**（GUI フレームワーク）を載せる。GUIX を ThreadX スレッドとして起動し、565rgb ディスプレイドライバを LTDC の tear-free ダブルバッファ + DMA2D に、入力ドライバを FT5336 に接続する。UI は **2 つの全画面ページ**（#61/#68）: **プレビューページ**（カメラ映像（既定 QVGA、#69/#84 で選択可）を等倍中央に表示する `GX_ICON` のみのクリーンなライブ映像）と、**ライブ映像をタップして開く設定ページ**（ライブ映像を出さない静的パネルに OV5640 の画質 9 種 + 解像度セレクタ（#69）+ Back を置く）。LTDC + GUIX エピック（#48）の Phase 4（#55）として導入し、#56 でカメラ合成、#60 で boot ON、#61 でカメラ UI 専用化、#68 で 2 ページ制御 UI 化、#69 でプレビュー解像度の切替を追加した。
...
!!! note "preview は base capture の subscriber（Epic #99 #100/#101/#102）"
    Epic #99 で GUI preview は DCMI を**所有しなくなった**。`camera_preview_start()` は廃止され、preview は `camera_subscribe(&guix_cam_sink, CAM_FMT_RGB565)` で **base capture（`camera stream`）の subscriber** として attach する（`camera_unsubscribe` で detach）。所有権の全体像は [所有権・状態モデル](../architecture/ownership.md)。要点:
    ...
    詳細な新モデルは [所有権・状態モデル](../architecture/ownership.md) を参照。
...
UI（`ui/guix_camera_ui.c`）は GUIX Studio を使わず手書き。色/フォント/pixelmap テーブルを手で組む。**2 画面**: root の兄弟子ウィンドウ（#55 の多画面 show/hide イディオム）として `preview_screen`（黒背景, `cam_icon`= `GX_ICON`, カメラ映像を等倍中央・解像度選択可 #69）と `settings_screen`（暗背景, 制御 9 行 + 解像度 + Back、既定 hidden, #68）を持ち、常にどちらか一方だけを表示する。全ウィジェット static なので GUIX のメモリアロケータは不要。
...
| SDRAM | canvas は増分なし（既存 LTDC ダブルバッファを流用） | カメラプレビュー（#56）は view バッファ +150 KB（320×240×2 QVGA、選択可能な最大解像度に合わせて確保、#69/#84） |
...
1. **push sink**（`guix_cam_sink`, `FRAME_POLICY_DROP`）を `camera_subscribe(&guix_cam_sink, CAM_FMT_RGB565)` で base capture に attach。producer スレッド（prio 10）上で `consume()` が呼ばれる。
2. `consume()` は ring slot を **DMA2D M2M でプライベート view バッファ `cam_view_buf`（最大 QVGA 320×240, `.sdram` 非キャッシュ。実コピーは選択解像度, #69/#84）へコピー**（`guix_display_copy_rgb565`, ltdc_lock 下）し、slot を即 `put`（同期完結 — pin をスレッド跨ぎ保持しない）。コピー成功時のみ coalesce フラグを立て `GX_EVENT_CAMERA_FRAME` を root へ post（`gx_system_event_send`, スレッドセーフ）。送信失敗時はフラグを立てず次フレームで再試行（freeze 回避）。
...
**所有権モデル（#100/#101: subscriber cascade）**: preview は DCMI を**所有しない** — `camera_subscribe(&guix_cam_sink, CAM_FMT_RGB565)` で **base capture（`camera stream`）の RGB565 subscriber** として attach する（`camera_unsubscribe` で detach）。`gui start` は base を起動せず、稼働中（RGB565）なら即 attach、そうでなければ enabled + idle で待機し次の `camera stream start` で attach、base off 中は最後のフレームで**凍結**。`gui stop` は unsubscribe のみ（**base も AI も止めない**）。base ON 中の `camera res/format/fps/probe/off` は base 側で `CAM_ERR_BUSY`（設定は `camera stream stop` 後）。DCMI overrun 等の **非同期 teardown（cascade）** は全 subscriber の `close()` を呼ぶが、preview は enabled のまま自動 re-attach する。

**自動起動と start/stop 競合（#61）**: preview の subscribe（probe を伴う geometry 同期）はブロッキング I2C を含み得るので、`camera_ui_start()` は `GX_EVENT_CAMERA_AUTOSTART` を post し、**GUIX スレッド**がそのハンドラで `preview_subscribe()`（内部で `camera_subscribe`）を実行する（boot/`gui start` 共通）。GUIX スレッドの start と shell スレッドの `gui stop` の競合は volatile フラグで防ぐ:

- AUTOSTART ハンドラ冒頭で `stop_requested || !guix_is_up()` なら no-op（`stop_requested` の clear は `camera_ui_start` の post 前のみ、ハンドラは clear しない）。
- `camera_ui_stop()` は `stop_requested=1` の後、**常に** `camera_unsubscribe(&guix_cam_sink)` + bounded drain を行う（base は継続）。
- boot は preview を subscribe した上で base を 1 回自動起動する（`base_autostarted`、out-of-box で live preview）。
...
- **プレビュー解像度（#69）**: `Resolution` サイクルボタン（160x120 / 320x240、いずれも RGB565 / 15fps — format と fps は固定）。画質と違い live な再フォーマットは `CAM_ERR_BUSY`（ring が live DMA target）なので、**stop → 再フォーマット → restart**: stream を停止して idle 待ち → view pixmap と `cam_icon` をリサイズ → `guix_display` のカメラ矩形 geometry を再アーム → 新解像度で再開（新映像は `Back` で表示）。どちらも 4:3 でパネル中央に配置。選択は `gui stop`/`gui start` を跨いで維持、GUI プレビューの boot 既定は **QVGA**（#84。shell の capture/stream は QVGA 既定のまま）。
...
**`ai stream` 稼働中はライブプレビュー上に顔検出の bbox 枠（緑）を常時描画**する（#100 で専用の `gui overlay` コマンドは廃止、`nn_camera_running()` で判定）。AI(nncam) は GUI preview と同じ base capture の **独立 subscriber** として RGB565 フレームを consume し BlazeFace-128（[AI 推論](../ai/inference.md)）へ流す。推論は **~1.5fps / prio18 best-effort**、プレビューは ~15fps を維持（box は推論完了ごとに更新）。worker の連続 bank3（活性化）アクセス（~93% CPU）は原理的には DCMI DMA の帯域を奪い overrun し得る（[#90](../ai/inference.md)）。

- **GUI と AI はデータのみ結合**: GUI は AI の起動/停止をしない — `nn_camera_dets_get()` の結果（顔 box）を read-only で読み、`nn_camera_running()` の間だけ描く。AI の有効化/無効化は `ai stream start`/`stop` の責務（両者は独立 subscriber、#100/#101）。
- **box は GUIX スレッドで「フレームの一部」として描く**: #59 B2 copy-forward は camera 矩形を毎 flip `cam_view_buf` から再スタンプするため、preview 上に別 widget を重ねると上書きされる。よって box は `guix_display_cam_overlay_box()` で **`cam_view_buf` に直接（`ltdc_lock` 下で）焼き込む**が、**producer(prio10) ではなく GUIX スレッドの `GX_EVENT_CAMERA_FRAME` ハンドラで描く**（producer で box ごとに `ltdc_lock` を取ると DCMI DMA の再アームを遅らせ overrun するため）。store / pixelmap draw / corrective refresh と同じ経路で両 LTDC バッファに伝播。box 座標は正規化 [0,1]（前処理が full frame を 128×128 に x/y 独立 squash）で clamp、次 store が古い box を上書き（`ai stream stop` / 検出消失で自然に消える）。`cam_view_buf` は bank0 non-cacheable SDRAM ゆえ cache maintenance 不要。
- **frame-perfect ではない**: overlay は最後に完了した推論結果を描くので、box は表示中フレームより数フレーム前の顔位置を指す（ライブ overlay として許容）。
- **overrun 自動復帰は base capture(producer)側**（#100）: DCMI overrun で base が倒れたら producer の `cam_stream_recover` が同モードで再アームし、preview/AI subscriber は close()/open() ペアを見て自動 re-attach。連続 overrun には escalating backoff（規定回数で自動復帰停止、`camera stream start` で復帰）。GUI 側の専用 backoff / `GX_EVENT_CAMERA_RESTART` は撤去。
- **480×272 は #84 で削除した**: 全画面 480×272 プレビューは横伸び（OV5640 が 4:3 センサ視野を 16:9 へ非一様スケール＝横に伸びる）と帯域過多の両方を抱えていた。store（DMA2D bank1→bank0, 261KB/frame）+ DCMI(bank1) + 推論(bank3) が DCMI を overrun し、**F746 の bus matrix は round-robin で per-master の SDRAM QoS を持たない**（RM0385 §2）ため優先度では解決不可だった。QVGA(320×240) はアスペクト正（4:3・伸びなし）で bank1 帯域も約 -41%。残る 2 モード（QQVGA/QVGA）は伸びず overrun もしない。

## 使い方

```text
# 電源投入で起動直後にカメラライブプレビューが出る（boot で GUI ON, #60/#61）
# 映像をタップ → 設定ページ（画質 9 種 + Resolution + Back）。Back でライブプレビューへ戻る（#68/#69）
# Resolution は 160x120 / 320x240 を循環（stop→再フォーマット→restart、#69）
...
sh> ai stream start   # AI 顔検出を有効化 → preview 上に顔 bbox を常時描画（#83、gui overlay は廃止）
```
