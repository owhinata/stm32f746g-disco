...
`gui overlay on` で**ライブプレビュー上に顔検出の bbox 枠（緑）を描画**する（`gui overlay off` で解除、既定 OFF、引数無しの `gui overlay` で状態と推論統計を表示）。プレビュー映像を BlazeFace-128（`ai` コマンドと同じ推論土台、[AI 推論](../ai/inference.md)）に feed し、返ってきた顔 box をプレビューに重ねる。推論は **~1.5fps / prio18 best-effort**、プレビューは ~15fps を維持（box は推論完了ごとに更新）。worker の連続 bank3（活性化）アクセス（~93% CPU）は原理的には DCMI DMA の帯域を奪い overrun し得るが、#84 以降の最大プレビューである QVGA は帯域に余裕があり overrun しない。下記の **overrun 自動復帰**は安全網として残す。
