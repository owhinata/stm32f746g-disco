 ...
 * being attempted.  This keeps a typo off the Reserved holes in the F746 memory
 * map (RM0385 §2.2.2) -- a read/write there faults to the weak Default_Handler,
 * whose infinite loop would hang the shell.  The default map permits all real
 * RAM/Flash plus the on-chip peripheral bus windows and the PPB; only Reserved
 * holes and Flash writes are blocked.  Peripheral/PPB are word-access only
 * (many registers fault or misbehave on sub-word access), so dump -- which is
 * byte-granular -- is allowed on RAM/Flash only.  Production hardening removes
 * devmem wholesale via CLI_ENABLE_DANGEROUS_CMDS=0.
 *
...
/*
 * Default region allow-list (RM0385 §2.2.2 memory map + peripheral bus windows).
 * Reserved holes are simply absent, so any access overlapping one is rejected.
 * Flash is the AXIM alias only and read-only (a plain store is not a Flash
 * programming sequence).  The on-chip peripheral buses and the PPB are word-only.
 */
static const struct devmem_region devmem_map[] = {
	{ 0x00000000u, 0x00004000u, 1, 1, WALL, "ITCM-RAM" }, /* 16 KB              */
	{ 0x08000000u, 0x00100000u, 1, 0, WALL, "Flash"    }, /* 1 MB, AXIM, RO     */
	{ 0x20000000u, 0x00010000u, 1, 1, WALL, "DTCM"     }, /* 64 KB              */
	{ 0x20010000u, 0x00040000u, 1, 1, WALL, "SRAM"     }, /* SRAM1+2, 256 KB    */
	{ 0x40000000u, 0x00008000u, 1, 1, W32,  "APB1"     },
