 ...
 * "disabled (build)".  `wdt starve` is a *dangerous* command (spec §12) AND needs
 * the watchdog actually built in, so it compiles only when both
 * CLI_ENABLE_DANGEROUS_CMDS and BSP_ENABLE_IWDG are set -- without the IWDG the
 * spin would just hang forever with nothing to reset the board.
 *
...
#include "stm32f7xx_hal.h"   /* __disable_irq */
 ...
 * over LSI tolerance) and resets the board.  Sleep first so the warning reaches
 * the VCP before IRQs go off (same TX-drain reasoning as `reboot`, cmd_system.c).
 */
